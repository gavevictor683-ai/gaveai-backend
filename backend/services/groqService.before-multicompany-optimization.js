...
- Do not assume that information found for one company applies to another company.
...
- When the user asks for sources, place the relevant source URL immediately after or below the corresponding information.`
    ...
    try {
      ...
      */
      ...
      if (!searchContext) {
        ...
        if (
          webSearchData &&
          Array.isArray(webSearchData.results)
        ) {
          collectedSources =
      ...
      */
      ...
      webSources = Array.isArray(collectedSources)
        ? collectedSources.slice(0, 8)
        : [];
      ...
      /*
      |--------------------------------------------------------------------------
      | ADD SEARCH RESULTS TO GROQ CONTEXT
      |--------------------------------------------------------------------------
      */
  ...
  */
  ...
  return {
    ...
    sources: Array.isArray(webSources)
